		...
		return {
			...
			link: function(scope, element, attributes) {
				...
				function onInit(editor, minder) {
					...
					let data = ipcRenderer.sendSync('render-message', {type: 'getData'});
					if(data && data.length > 0){
						editor.minder.importJson(JSON.parse(data));
					}
					editor.minder.on('contentchange', function() {
						let success = ipcRenderer.sendSync('render-message', {type: 'changeData', data: JSON.stringify(editor.minder.exportJson())});
						if(success && document.title.indexOf('*') < 0){
